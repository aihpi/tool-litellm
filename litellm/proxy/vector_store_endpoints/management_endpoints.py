...
from litellm.proxy.vector_store_endpoints.utils import check_vector_store_permission
...
async def create_vector_store_in_db(
    ...
    if litellm_params:
        litellm_params_dict = GenericLiteLLMParams(**litellm_params).model_dump(exclude_none=True)
    ...
    # Create in database
    _new_vector_store = await ManagedVectorStoresRepository(prisma_client).table.create(data=data_to_create)

    new_vector_store: LiteLLM_ManagedVectorStore = LiteLLM_ManagedVectorStore(**_new_vector_store.model_dump())
...
def _resolve_qdrant_connection_params(
    ...
    resolved_params: Dict[str, Any] = {key: _resolve_secret_value(value, key) for key, value in litellm_params.items()}

    api_base = resolved_params.get("api_base") or get_secret("os.environ/QDRANT_API_BASE")
    if not api_base:
    ...
    collection_name = (
        vector_store_metadata.get("qdrant_collection_name") if isinstance(vector_store_metadata, dict) else None
...
async def _resolve_cached_embedding_config_with_secrets(
    embedding_model: str,
    ...
    if config is None:
        config = await _resolve_embedding_config_from_db(embedding_model=embedding_model, prisma_client=prisma_client)
    if not config:
        return None
...
async def new_vector_store(
    ...
    try:
        ...
        if user_api_key_dict.user_id and not validated_metadata.get("created_by"):
            validated_metadata["created_by"] = user_api_key_dict.user_id
        if user_api_key_dict.team_id and not validated_metadata.get("team_id"):
            validated_metadata["team_id"] = user_api_key_dict.team_id
...
async def create_qdrant_collection(
    ...
    collection_name = request.collection_name or vector_store_id
    ...
    if not (
        ...
        if not has_permission:
            raise HTTPException(
                status_code=403,
                detail="User does not have permission to create this vector store collection.",
            )
    ...
    distance_map = {"cosine": "Cosine", "dot": "Dot", "euclid": "Euclid"}
    payload: Dict[str, Any] = {
        "vectors": {
            "size": request.vector_size,
            "distance": distance_map[request.distance],
        }
    }

    if request.collection_config:
        payload.update(request.collection_config)

    headers = {"Content-Type": "application/json"}
    if api_key:
        headers["api-key"] = api_key

    url = f"{api_base.rstrip('/')}/collections/{collection_name}"
    try:
        ...
        try:
            ...
            verbose_proxy_logger.warning("Failed to persist qdrant collection name metadata: %s", metadata_error)
...
async def _get_embeddings_for_texts(
    *,
    ...
    if llm_router is not None:
        response = await llm_router.aembedding(model=embedding_model, input=texts, **embedding_kwargs)
    else:
        response = await litellm.aembedding(model=embedding_model, input=texts, **embedding_kwargs)
...
async def upsert_qdrant_points(
    ...
    if custom_llm_provider != "qdrant":
        raise HTTPException(status_code=400, detail="Only Qdrant vector stores are supported.")
    ...
    embedding_config = await _resolve_cached_embedding_config_with_secrets(
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.put(url, headers=headers, json={"points": points}, params={"wait": "true"})
        if response.status_code >= 400:
            raise HTTPException(status_code=response.status_code, detail=response.text)
        return {
            "status": "success",
...
async def list_vector_stores(
    ...
    try:
        ...
        vector_stores_from_db = await VectorStoreRegistry._get_vector_stores_from_db(prisma_client=prisma_client)
        ...
        if existing_vector_store is not None:
            ...
            if custom_llm_provider.lower() == "qdrant":
                ...
                resolved_params: Dict[str, Any] = {
                    key: _resolve_secret_value(value, key) for key, value in litellm_params.items()
                }
                ...
                if api_base:
                    ...
                    try:
                        async with httpx.AsyncClient() as client:
                            response = await client.delete(delete_url, headers=delete_headers)
                        if response.status_code >= 400:
