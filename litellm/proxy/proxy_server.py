...
@router.post("/v2/login/ldap", include_in_schema=False)  # hidden helper for UI LDAP logins via API
async def login_v2_ldap(request: Request):  # noqa: PLR0915
    global premium_user, general_settings, master_key
    ...
    try:
        ...
        login_result = await authenticate_ldap_user(
            username=username,
            password=password,
            master_key=master_key,
            prisma_client=prisma_client,
        ...
        verbose_proxy_logger.exception(
            "litellm.proxy.proxy_server.login_v2_ldap(): Exception occurred - {}".format(str(e))
        )
        if isinstance(e, ProxyException):
            raise e
        elif isinstance(e, HTTPException):
